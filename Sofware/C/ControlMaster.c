//Compilar:
//gcc ControlMaster.c -o /home/pi/Ejecutables/controlmaster -lbcm2835 -lwiringPi 
//gcc ControlMaster.c -o controlmaster -lbcm2835 -lwiringPi 
...
#define TEST 29 																//Pin 40 GPIO																//Pin 38 GPIO						
...
void ObtenerTiempoPIC();														//C:0xA5	F:0xF5    P:0xB2
...
void ObtenerOperacion(){
	...
	switch (buffer){                                                                     
          ...
          case 0xB2:
               ...
               break;
		  case 0xB3:
		       printf("Recupero 0xB3\n"); 
			   RecuperarTramaPrueba();  
               break;
          default:
...
void CheckComunicacionRS485(){
	printf("Comprobando comunicacion RS485...\n");
...
void RecuperarTramaPrueba(){
	printf("Trama de prueba:\n");	
	bcm2835_spi_transfer(0xA9);                                                 //Envia el delimitador de final de trama
    bcm2835_delayMicroseconds(TIEMPO_SPI);
	for (i=0;i<10;i++){
        buffer = bcm2835_spi_transfer(0x00);
        tramaPrueba[i] = buffer;												//Recupera los 10 bytes de la trama de prueba
        bcm2835_delayMicroseconds(TIEMPO_SPI);
    }
	bcm2835_spi_transfer(0xF9);                                                 //Envia el delimitador de final de trama
    bcm2835_delayMicroseconds(TIEMPO_SPI);
	printf("[ ");	
	for (i=0;i<10;i++){
        printf("%0.2d ",tramaPrueba[i]);										//Imprime la trama de prueba recuperada
    }
    printf("]\n");	
}
