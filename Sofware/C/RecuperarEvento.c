//Compilar:
//gcc RecuperarEvento.c -o recuperarevento -lbcm2835 -lwiringPi 
...
unsigned short banSolicitud, contSolicitud, contSegundos, banLeerAcel;
...
void ImprimirInspeccionSector(unsigned char* pyloadRS485);
	...
	switch (funcionSPI){                                                                     
          ...
          case 0xB3:
			   ...
			   if (subFuncionSPI==0xD2){
				   ...
				   ImprimirInspeccionSector(tramaPyloadRS485);
			   }
...
void ImprimirInspeccionSector(unsigned char* pyloadRS485){
	...
	if ((pyloadRS485[1]==0xFF)&&(pyloadRS485[2]==0xFD)&&(pyloadRS485[3]==0xFB)){
		...
		contSolicitud = 0;
		//Activa la bandera para proceder a la lectura de la aceleracion a partir del sector actual
		banLeerAcel = 1;
