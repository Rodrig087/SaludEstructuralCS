...
#Estilo de graficos: 0 = modo normal, 1 = modo paper
modoGrafico = 0;    

#Ingreso de datos:
nombreEstacion = "Sismo 2021-04-26"
...
fechaInfo = str('%0.2d' % tramaDatos[tramaSize-6]) + str('%0.2d' % tramaDatos[tramaSize-5]) + str('%0.2d' % tramaDatos[tramaSize-4])
horaInfo = str('%0.2d' % (int(horaInicioSeg/3600))) + str('%0.2d' % (int((horaInicioSeg%3600)/60))) + str('%0.2d' %((horaInicioSeg%3600)%60))
...
if (banExtraer==1):
                            ...
                            if (contEje==2):
                                ...
                                if (numPiso==0):
                                    if (numNodo==1 or numNodo==2):
                                        ...
                                        xTrama.append(acelY-offTran)
                                        yTrama.append(acelZ-offLong)
                                        zTrama.append(acelX-offVert)
                                        casoNodo = 1
                                    if (numNodo==3):
                                        #ZnYn-Xn:
                                        #fprintf(fileX, "%2.5f\t%2.5f\t%2.5f\n", acelZ, acelY, acelX*-1)
                                        xTrama.append(acelZ-offTran)
                                        yTrama.append(acelY-offLong)
                                        zTrama.append((-1*acelX)-offVert)
                                        casoNodo = 2
                                    if (numNodo==4):
                                        #-ZnYnXn:
                                        #fprintf(fileX, "%2.5f\t%2.5f\t%2.5f\n", acelZ*-1, acelY, acelX)
                                        xTrama.append((-1*acelZ)-offTran)
                                        yTrama.append(acelY-offLong)
                                        zTrama.append(acelX-offVert)
                                        casoNodo = 3
                                else:
    ...
    if (extraerEventoTxt=='s'):
        ...
        pathTxt = 'EventosTxt' + "/" + str(fechaEvento)
        try:
            os.mkdir(pathTxt)
        except OSError as e:
        ...
        datosInfo = fechaInfo + ',' + horaInfo + ',' + str(tiempoInicio) + ',' + str(duracionEvento) + ',4'
