...
;sdcard.c,90 :: 		unsigned char SD_Read(unsigned char *Buffer, unsigned int nbytes){
;sdcard.c,93 :: 		for(i = 0; i < SD_TIME_OUT; i++){
...
; i start address is: 4 (W2)
	...
	GOTO	L_SD_Read1
L__SD_Read78:
;sdcard.c,94 :: 		temp = SPISD_Write(0xFF);
	PUSH	W2
	PUSH.D	W10
	MOV.B	#255, W10
	CALL	_SPISD_Write
	POP.D	W10
	POP	W2
;sdcard.c,95 :: 		if(temp == 0xFE) break;
	MOV.B	#254, W1
	CP.B	W0, W1
	BRA Z	L__SD_Read79
	GOTO	L_SD_Read3
...
; i end address is: 4 (W2)
	GOTO	L_SD_Read1
L_SD_Read3:
;sdcard.c,96 :: 		if(i == SD_TIME_OUT-1) return TOKEN_NOT_RECEIVED;
; i start address is: 4 (W2)
	MOV	#1999, W0
	CP	W2, W0
	BRA Z	L__SD_Read80
...
; i end address is: 4 (W2)
	GOTO	L_SD_Read0
L_SD_Read1:
;sdcard.c,98 :: 		for(i = 0; i < nbytes; i++){
; i start address is: 4 (W2)
	CLR	W2
; i end address is: 4 (W2)
L_SD_Read5:
; i start address is: 4 (W2)
	...
	GOTO	L_SD_Read6
L__SD_Read81:
;sdcard.c,99 :: 		Buffer[i] = SPISD_Write(0xFF);
	ADD	W10, W2, W0
	MOV	W0, [W14+0]
	PUSH	W2
	PUSH.D	W10
...
; i end address is: 4 (W2)
	GOTO	L_SD_Read5
L_SD_Read6:
;sdcard.c,101 :: 		temp = SPISD_Write(0xFF);     // Read 16bits of CRC
	PUSH.D	W10
	MOV.B	#255, W10
	CALL	_SPISD_Write
;sdcard.c,102 :: 		temp = SPISD_Write(0xFF);     //
	MOV.B	#255, W10
	CALL	_SPISD_Write
	POP.D	W10
;sdcard.c,103 :: 		return 0x00;                // Successful read
	CLR	W0
;sdcard.c,104 :: 		}
L_end_SD_Read:
	ULNK
...
;sdcard.c,115 :: 		unsigned char SD_Read_Block(unsigned char *Buffer, unsigned long Address){
;sdcard.c,117 :: 		Select_SD();
	PUSH	W11
	PUSH	W12
	PUSH	W13
	CALL	_Select_SD
;sdcard.c,119 :: 		if(ccs == 0x02) Address<<=9;        // Address * 512 for SDSC cards
	MOV	#lo_addr(_ccs), W0
	MOV.B	[W0], W0
	CP.B	W0, #2
...
L__SD_Read_Block83:
	...
	MOV	W1, W12
L_SD_Read_Block8:
;sdcard.c,120 :: 		if(SD_Ready() == 0) return SD_NOT_READY;
	PUSH	W11
	PUSH	W12
	PUSH	W10
	CALL	_SD_Ready
...
L_SD_Read_Block9:
;sdcard.c,121 :: 		SD_Send_Command(READ_SINGLE_BLOCK,Address,0xFF);
	PUSH	W10
	MOV.B	#255, W13
	MOV.B	#17, W10
	CALL	_SD_Send_Command
;sdcard.c,122 :: 		temp = R1_Response();
	CALL	_R1_Response
	POP	W10
; temp start address is: 2 (W1)
	MOV.B	W0, W1
;sdcard.c,123 :: 		if(temp != 0x00) return temp;
	CP.B	W0, #0
...
; temp end address is: 2 (W1)
	GOTO	L_end_SD_Read_Block
L_SD_Read_Block10:
;sdcard.c,124 :: 		temp = SD_Read(Buffer,512);
	MOV	#512, W11
	CALL	_SD_Read
; temp start address is: 2 (W1)
	MOV.B	W0, W1
;sdcard.c,126 :: 		Release_SD();
	CALL	_Release_SD
;sdcard.c,127 :: 		return temp;
	MOV.B	W1, W0
; temp end address is: 2 (W1)
;sdcard.c,128 :: 		}
;sdcard.c,127 :: 		return temp;
...
;sdcard.c,327 :: 		unsigned int R2_Response(void){
;sdcard.c,330 :: 		temp = SPISD_Write(0xFF);
	PUSH	W10
	MOV.B	#255, W10
	CALL	_SPISD_Write
;sdcard.c,331 :: 		response = SPISD_Write(0xFF);
	MOV.B	#255, W10
	CALL	_SPISD_Write
	ZE	W0, W0
	MOV	W0, [W14+0]
;sdcard.c,332 :: 		temp = SPISD_Write(0xFF);
	MOV.B	#255, W10
	CALL	_SPISD_Write
;sdcard.c,333 :: 		response = (response<<8)|temp;
	MOV	[W14+0], W1
	SL	W1, #8, W1
	ZE	W0, W0
	IOR	W1, W0, W0
;sdcard.c,334 :: 		return response;
;sdcard.c,335 :: 		}
;sdcard.c,334 :: 		return response;
;sdcard.c,335 :: 		}
L_end_R2_Response:
